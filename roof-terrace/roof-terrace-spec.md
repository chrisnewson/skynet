...
**The starting point was positive.** At pre-app the officers concluded the proposal was *"likely to be considered acceptable, in terms of its impact on the grade I listed building and neighbouring amenity."* They placed **almost everything in the "Acceptable" column** — the removal of the timber patio, the porcelain tiles, the difference in levels, the steel beams, the railings (a *"modern addition … no historic significance,"* which they said could even be replaced entirely in timber or glass), the guarding around the western parapet, the hot tub (*"would not be visible from the street"*), and the fully-retractable parasols. They flagged **one** item as *"Needs Attention."* The refined scheme keeps everything they accepted and reworks that one element.
...
## Answering the one point the officers said *Needs Attention*

The whole of their "Needs Attention" section was a **single** issue — the sauna seen from Bristol Gardens:

> *"The sauna would be visible from vantage points along Bristol Gardens. Its location/design should ideally be altered to minimise its visual impact."*

They offered two routes to that: **(a)** replace the combined sauna/store with **two separate structures** — a sauna *"reduced in width"* plus *"a separate store of a lower height … adjacent to the southern parapet wall"* — **or (b)** *"move the sauna further away from the … parapet wall."* Heritage Officer Lance Penman put the same point as a setback: *"set further back by about 3–4 m from the parapet to the Sussex Row parapet … to avoid it being prominently visible from that street."* **The refined design takes both routes, and goes further than either:**

- **Set back 3 m from the rear (Bristol Gardens / Sussex Row) parapet** — up from **0.6 m** at pre-app. This directly meets Lance Penman's *"3–4 m"* request and is the single biggest reduction in streetscene prominence.
- **The store has left the roofline altogether.** Rather than just making the store *"lower"* and *"separate"* as suggested, storage moves into **low outdoor cabinets sitting below parapet height** (Part G) — so there is **no store on the elevated structure at all**, only the sauna/wet-room mass remains above the parapet.
- **The mass is reshaped and shrunk in elevation.** The pre-app sauna/store ran ~4.5 m along the parapet; the refined building presents a **near-square footprint with its longest face cut by about a third** (≈2.9 m), within the **same ~7.2 m² enclosed area** the officers already accepted — so it reads as a small box pulled back from the edge, not a long wall along the skyline.
- **Low, flat, recessive profile.** An essentially **flat roof** (3–5° drainage fall only) with a **low-profile bird-wire grid** instead of any bulky parapet element, finished in **anthracite PVDF marine-aluminium standing seam** — durable, low-reflectance and quiet against the brick flank wall. *(At the pre-app meeting the officers indicated a preference for a more permanent finish than plain weathering timber; this aligns with that steer.)*

**One point to make honestly to planning — the height.** The flat roof now sits at **~1,293 mm above the parapet**, **~143 mm higher than the 1,150 mm shown at pre-app**. The increase is **not the building growing** — it comes from raising the finished floor level when the whole main roof section was inverted (a roofing-robustness improvement agreed with IKO and the contractors, Part A). In visual terms this small rise is **far outweighed by the ~2.4 m of extra setback and the third-shorter elevation**, so net prominence from Bristol Gardens is **reduced**, not increased — but we will **re-present the revised height openly**. *(Ronan: the figure should come down a little once recomputed on the confirmed 190 mm insulation — please confirm the final number before submission.)*

**A second honest point — the setback moves the hot tub towards the north parapet, so glass guarding is now needed there.** Pulling the building back 3 m from the north (Bristol Gardens / Sussex Row) parapet — the change the officers specifically asked for — uses up the depth behind the sauna, so the **hot tub now sits in the space between the sauna and the north parapet**, closer to that edge than before. That triggers a **fall-protection requirement along the hot-tub runs at the north/east edge** (plus the west dining edge). We're flagging this openly as a **direct consequence of the setback the officers requested** — but the guarding is **frameless toughened glass, point-fixed to the *inner* face of the parapet, with no metal cap-rail at the top and no penetration of the coping or roof** (Part L, Phase 2). **Glass reads as an almost-invisible sheet, so its impact on the Bristol Gardens streetscene is very low** — far smaller than the visual gain of setting the building back. The officers had already indicated that guarding around the western parapet, and a glass balustrade, would be acceptable.

**Other changes, all reducing impact:**

- **Less intervention in the historic structure.** The pre-app proposed a lattice of new steel members fixed into the existing parapet to carry the hot tub. On the structural engineer's advice we have adopted a lighter solution — **doubling the timber joists** beneath the hot tub — reducing new steelwork in the listed fabric.
- **Redundant railings removed.** The central and short connecting railings are redundant under the refined layout and have been removed, leaving the east and west railings in place with **discreet stainless guarding wires** on top. The officers raised **no objection** to altering these modern, non-historic railings (and confirmed a slight height increase, or even full replacement in timber or glass, would be acceptable) — so one less element breaks the rear skyline.
- **Parasols.** Both are **fully retractable** and furled when not in use, so they are **not conspicuously visible above the parapet** in their normal state — directly meeting Lance Penman's condition; and the **southern parasol is rotated 180°** as Charlie Partridge suggested (Part P).
- **Biodiversity improved, not just protected.** A bare asphalt roof becomes a densely planted terrace — pollinator-friendly planters, 4 olive trees and a coastal palette, with bee bricks in the storage — supporting the **measurable Biodiversity Net Gain** and the **Biodiversity Checklist** the officers require (see the submission note below and Part D).

**Net effect:** the same accepted footprint, pulled back and lowered in prominence; the store and the plain timber gone; less new steel in the historic structure; the redundant railings cleared; the fall-protection glass all but invisible; and a bare roof turned green. This is not a departure from the pre-app design — it is that design improved along exactly the lines the officers set out, and remains comfortably *less than substantial harm*.

## Information the officers require with the application

Beyond the standard validation drawings, the pre-app letter asks for **two specific items** — both must be ready before the application goes in:

> **Ronan — two actions for the submission:**
>
...
How our tile finish bears on the **inverted main section** still needs IKO's confirmation. IKO's spec contemplates *"paving slabs on supports, laid over the WCL"* — so pedestals on the inverted finish are allowed in principle (here on the **Protectoboard laid over the WCL** — see the build-up above) — but three things need pinning down:
...
- **Inverted main section** — the Permaphalt is at the **bottom** (on the deck), **buried and protected under the 190mm XPS insulation**, so there's no asphalt at the surface to "board over." Over the insulation the **IKO Enertherm WCL is laid first, then the IKO Protectoboard over it**, with the paving supports on the Protectoboard. The membrane is protected by being below the insulation.
...
> **Planning response (pre-app PRE2025/00255).** Both parasols are **fully retractable** and furled when not in use, so they are **not conspicuously visible above the parapet** in their normal state — meeting Heritage Officer Lance Penman's condition that parasols *"must not be permanent and must be of a scale and design that makes it easily retractable and not conspicuously visible above the parapet."* The **southern parasol is rotated 180° from the orientation shown at pre-app**, as Planning Officer Charlie Partridge suggested (*"the southern parasol could be rotated 180°"*), to reduce its visual impact from the streetscene — Ronan to fix the exact mast orientation on the submission drawings. Both points are made in the [Planning Case](#planning-case--why-the-refined-scheme-merits-support).
...
- On the inverted main, lay the **Enertherm WCL onto the XPS first, then the Protectoboard over it** (ref 2.0, per Ronan's GA300) — keep this in the IKO spec so the 20-year guarantee stands.
